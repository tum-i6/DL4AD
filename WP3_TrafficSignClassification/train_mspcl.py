...
def train_batch(model, model_name, loss_func, x, y, opt, optimizer_prox_2,
            optimizer_prox_1, optimizer_conprox_2, optimizer_conprox_1, criterion_prox_2,
            criterion_prox_1, criterion_conprox_2, criterion_conprox_1, custom_clusters, config):
    """
    ...
    loss_prox_1 = criterion_prox_1(feats1, y_custom)
    loss_prox_2 = criterion_prox_2(feats2, y_custom)

    loss_conprox_1 = criterion_conprox_1(feats1, y_custom)
    loss_conprox_2 = criterion_conprox_2(feats2, y_custom)

    custom_loss_prox_1, custom_loss_prox_2, custom_loss_conprox_1, custom_loss_conprox_2=PCL_custom_loss(custom_clusters, feats1, feats2, y, config,
                                            criterion_prox_1.centers, criterion_prox_2.centers,
                                            criterion_conprox_1.centers, criterion_conprox_2.centers)

    #---------old version- in the cvpr paper
    #loss_prox_1=(loss_prox_1+custom_loss_prox_1)/2
    #loss_prox_2=(loss_prox_2+custom_loss_prox_2)/2

    #loss_conprox_1 = (loss_conprox_1 + custom_loss_conprox_1) / 2
    #loss_conprox_2 = (loss_conprox_2 + custom_loss_conprox_2) / 2

    #--------new version
    loss_prox_1 = loss_prox_1 + custom_loss_prox_1
    loss_prox_2 = loss_prox_2 + custom_loss_prox_2

    loss_conprox_1 = loss_conprox_1 + custom_loss_conprox_1
    loss_conprox_2 = loss_conprox_2 + custom_loss_conprox_2

    loss_prox_1 *= config['pcl_weight-prox']
    loss_prox_2 *= config['pcl_weight-prox']

    loss_conprox_1 *= config['pcl_weight-conprox']
    loss_conprox_2 *= config['pcl_weight-conprox']

    loss=loss + loss_prox_1 + loss_prox_2  - loss_conprox_1 - loss_conprox_2 # total loss
    optimizer_prox_1.zero_grad()
    optimizer_prox_2.zero_grad()

    optimizer_conprox_1.zero_grad()
    optimizer_conprox_2.zero_grad()
    loss.backward()
    opt.step()

    for param in criterion_prox_1.parameters():
        param.grad.data *= (1. / config['pcl_weight-prox'])
    optimizer_prox_1.step()

    for param in criterion_prox_2.parameters():
        param.grad.data *= (1. / config['pcl_weight-prox'])
    optimizer_prox_2.step()

    for param in criterion_conprox_1.parameters():
        param.grad.data *= (1. / config['pcl_weight-conprox'])
    optimizer_conprox_1.step()

    for param in criterion_conprox_2.parameters():
        param.grad.data *= (1. / config['pcl_weight-conprox'])
    optimizer_conprox_2.step()

    PCL_step_custom_prox(custom_clusters, config)
    with torch.no_grad():
...
def train_epoch(model, model_name, train_loader, epoch, num_epochs, criterion, opt, optimizer_prox_2,
            optimizer_prox_1, optimizer_conprox_2, optimizer_conprox_1, criterion_prox_2,
            criterion_prox_1, criterion_conprox_2, criterion_conprox_1, custom_clusters, config):
    """
    ...
    with tqdm(train_loader, desc=f'{model_name} [Epoch:{epoch + 1}/{num_epochs}]') as pbar:
        ...
        for x, y in pbar:
            ...
            acc, corrects, loss, nums = train_batch(model, model_name, criterion, x, y, opt, optimizer_prox_2,
            optimizer_prox_1, optimizer_conprox_2, optimizer_conprox_1, criterion_prox_2,
            criterion_prox_1, criterion_conprox_2, criterion_conprox_1, custom_clusters, config)
            with torch.no_grad():
...
def train_model(num_epochs, patience, model, model_name, criterion, opt, train_loader, val_loader, session_path,
                train_writer, val_writer, optimizer_prox_2, optimizer_prox_1, optimizer_conprox_2,
                optimizer_conprox_1, criterion_prox_2, criterion_prox_1, criterion_conprox_2,
                criterion_conprox_1,custom_clusters, config):
    """
    ...
    for epoch in range(num_epochs):
        ...
        train_acc, train_loss = train_epoch(model, model_name, train_loader, epoch, num_epochs, criterion, opt, optimizer_prox_2,
            optimizer_prox_1, optimizer_conprox_2, optimizer_conprox_1,criterion_prox_2,
            criterion_prox_1, criterion_conprox_2, criterion_conprox_1,custom_clusters, config)

        # Evaluates the model using the validation dataset and returns the average accuracy and loss
        val_acc, val_loss = validate_epoch(model,model_name, val_loader, criterion,criterion_prox_2,
            criterion_prox_1, criterion_conprox_2, criterion_conprox_1)
...
def main():
    ...
    # PCL Trainig
    criterion_prox_1 = Proximity(num_classes=9, feat_dim=cfg['cpl_crit_1_sizes'][cfg['model_name']], use_gpu=True)
    criterion_prox_2 = Proximity(num_classes=9, feat_dim=cfg['cpl_crit_2_sizes'][cfg['model_name']], use_gpu=True)

    criterion_conprox_1 = Con_Proximity(num_classes=9, feat_dim=cfg['cpl_crit_1_sizes'][cfg['model_name']], use_gpu=True)
    criterion_conprox_2 = Con_Proximity(num_classes=9, feat_dim=cfg['cpl_crit_2_sizes'][cfg['model_name']], use_gpu=True)
    custom_clusters=PCL_custom_proxs(cfg)
    ...
    optimizer_prox_1 = torch.optim.SGD(criterion_prox_1.parameters(), lr=0.5, weight_decay=cfg['weight_decay'])
    optimizer_prox_2 = torch.optim.SGD(criterion_prox_2.parameters(), lr=0.5, weight_decay=cfg['weight_decay'])
    optimizer_conprox_1 = torch.optim.SGD(criterion_conprox_1.parameters(), lr=0.0001, weight_decay=cfg['weight_decay'])
    optimizer_conprox_2 = torch.optim.SGD(criterion_conprox_2.parameters(), lr=0.0001, weight_decay=cfg['weight_decay'])
