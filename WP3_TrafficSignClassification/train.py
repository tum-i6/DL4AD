'Train in CE, PCL and WPCL modes!'
import warnings
...
def train_batch(model, model_name, loss_func, x, y, opt, optimizer_prox_256,
            optimizer_prox_1024, optimizer_conprox_256, optimizer_conprox_1024, criterion_prox_256,
            criterion_prox_1024, criterion_conprox_256, criterion_conprox_1024, config):
    """
    ...
    if model_name == 'InceptionV3':
        feats256, feats1024, output, aux_output = model(x)  # Infers the training batch into the model
        ...
        feats256, feats1024, output = model(x)  # Infers the training batch into the model
        loss = loss_func(output, y)  # Calculates the loss
    if config['pcl_active']:
        loss_prox_1024 = criterion_prox_1024(feats1024, y)
        loss_prox_256 = criterion_prox_256(feats256, y)

        loss_conprox_1024 = criterion_conprox_1024(feats1024, y, custom_distance=config['pcl_custom_distance'], custom_dist_rate=config['pcl_custom_distance_rate'])
        loss_conprox_256 = criterion_conprox_256(feats256, y, custom_distance=config['pcl_custom_distance'], custom_dist_rate=config['pcl_custom_distance_rate'])

        loss_prox_1024 *= config['pcl_weight-prox']
        loss_prox_256 *= config['pcl_weight-prox']

        loss_conprox_1024 *= config['pcl_weight-conprox']
        loss_conprox_256 *= config['pcl_weight-conprox']
...
def train_epoch(model, model_name, train_loader, epoch, num_epochs, criterion, opt, optimizer_prox_256,
            optimizer_prox_1024, optimizer_conprox_256, optimizer_conprox_1024, criterion_prox_256,
            criterion_prox_1024, criterion_conprox_256, criterion_conprox_1024, config):
    """
    ...
    #with tqdm(train_loader) as pbar:
        ...
        for x, y in pbar:
            ...
            acc, corrects, loss, nums = train_batch(model, model_name, criterion, x, y, opt, optimizer_prox_256,
            optimizer_prox_1024, optimizer_conprox_256, optimizer_conprox_1024, criterion_prox_256,
            criterion_prox_1024, criterion_conprox_256, criterion_conprox_1024, config)
            with torch.no_grad():
...
def train_model(num_epochs, patience, model, model_name, criterion, opt, train_loader, val_loader, session_path,
                train_writer, val_writer, optimizer_prox_256, optimizer_prox_1024, optimizer_conprox_256,
                optimizer_conprox_1024, criterion_prox_256, criterion_prox_1024, criterion_conprox_256,
                criterion_conprox_1024, config):
    """
    ...
    for epoch in range(num_epochs):
        ...
        train_acc, train_loss = train_epoch(model, model_name, train_loader, epoch, num_epochs, criterion, opt, optimizer_prox_256,
            optimizer_prox_1024, optimizer_conprox_256, optimizer_conprox_1024,criterion_prox_256,
            criterion_prox_1024, criterion_conprox_256, criterion_conprox_1024, config)

        # Evaluates the model using the validation dataset and returns the average accuracy and loss
        val_acc, val_loss = validate_epoch(model,model_name, val_loader, criterion,criterion_prox_256,
            criterion_prox_1024, criterion_conprox_256, criterion_conprox_1024)
...
def main():
    ...
    parser.add_argument('--cdr', type=int, default=10,
                        help='the wpcl lambda rate'
                        )
    ...
    cfg['pcl_custom_distance_rate']=args.cdr
    #cfg['device_ids']=args.gpus
    ...
    session_name = f"{str(datetime.now())[:-7].replace(' ', '_').replace(':', '-')}_{cfg['model_name']}_lambda_{cfg['pcl_custom_distance_rate']}"
    ...
    # PCL Trainig
    cpl_crit_1 = Proximity(num_classes=cfg['num_classes'], feat_dim=cfg['cpl_crit_1_sizes'][cfg['model_name']], use_gpu=True)
    cpl_crit_2 = Proximity(num_classes=cfg['num_classes'], feat_dim=cfg['cpl_crit_2_sizes'][cfg['model_name']], use_gpu=True)

    criterion_conprox_1024 = Con_Proximity(num_classes=cfg['num_classes'], feat_dim=cfg['cpl_crit_1_sizes'][cfg['model_name']], use_gpu=True, dataset=cfg['dataset'])
    criterion_conprox_256 = Con_Proximity(num_classes=cfg['num_classes'], feat_dim=cfg['cpl_crit_2_sizes'][cfg['model_name']], use_gpu=True, dataset=cfg['dataset'])
    ...
    optimizer_prox_1024 = torch.optim.SGD(cpl_crit_1.parameters(), lr=0.5, weight_decay=cfg['weight_decay'])
    optimizer_prox_256 = torch.optim.SGD(cpl_crit_2.parameters(), lr=0.5, weight_decay=cfg['weight_decay'])
    optimizer_conprox_1024 = torch.optim.SGD(criterion_conprox_1024.parameters(), lr=0.0001, weight_decay=cfg['weight_decay'])
    optimizer_conprox_256 = torch.optim.SGD(criterion_conprox_256.parameters(), lr=0.0001, weight_decay=cfg['weight_decay'])
    ...
    best_train_acc, best_train_loss, best_val_acc, best_val_loss = \
        train_model(
            ...
            optimizer_prox_256,
            optimizer_prox_1024,
            optimizer_conprox_256,
            optimizer_conprox_1024,
            cpl_crit_2,
            cpl_crit_1,
            criterion_conprox_256,
            criterion_conprox_1024, cfg
        )
