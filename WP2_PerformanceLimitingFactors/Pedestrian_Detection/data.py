...
def load_citypersons_dataloaders(cfg):
    ...
    contain bounding box annotations. Note that the original CityPersons test dataset does not have public labels,
    ...
    val_dataset = CityPersons_dataset(cfg['data_path'])
    test_dataset = CityPersons_dataset(cfg['data_path'])
...
def load_kia_dataloaders(cfg):
    ...
    for sample_idx, img_sample_path in enumerate(train_dataset.imgs):
        ...
        sequence = int(sample_info[1].split('_')[3].split('-')[0].lstrip('0'))
...
class CityPersons_dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_path, transforms=get_transform(False), min_obj_pixels=None, max_occl=None, max_distance=None,
                 instance_segm=False):
        """
    ...
    def __getitem__(self, idx):
        ...
        represents a group of people (1) or just a single pedestrian instance (0), masks (key) -> An integer tensor of
        ...
        instances, bboxes, class_ids, bbox_areas, ignore, iscrowd = self.extract_2D_bbox_data(bbox_json_path, img)
    ...
    def extract_2D_bbox_data(self, json_path, img):
        ...
        for obj in labels['objects']:
            ...
            if obj['label'] != 'ignore':
                ...
                instances.append(str(obj['instanceId']))  # Stores the instance id
                bboxes.append([x1, y1, x2, y2])  # Stores the bbox coordinates
                class_ids.append(1)  # Stores the class information
                bbox_areas.append((x2-x1) * (y2-y1))  # Stores the bounding box area information
                ignore.append(int(obj['distance'] > 50))  # Ignore instances with a distance > 50m
                distances.append(obj['distance'])  # Stores the distance information
                iscrowd.append(int(obj['label'] == 'person group'))  # Stores the information about crowded labels
        ...
        for box_pair in list(itertools.combinations(range(len(bboxes)), 2)):
            ...
            if inter_area == 0:
                continue  # No intersection -> overlap is 0
            # Computes the overlap
            overlap1 = inter_area / bbox_areas[box_pair[0]]
            overlap2 = inter_area / bbox_areas[box_pair[1]]
            ...
            if overlap1 >= 0.6 or overlap2 >= 0.6:
                ...
                if distances[box_pair[0]] > distances[box_pair[1]]:
                    ignore[box_pair[0]] = 1
                else:
                    ignore[box_pair[1]] = 1

        return instances, bboxes, class_ids, bbox_areas, ignore, iscrowd
...
class KIA_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Extracts the 2D bbox information from the annotation json file
        instances, bboxes, class_ids, bbox_areas, ignore = self.extract_2D_bbox_data(bbox_json_path, img)
    ...
    def extract_2D_bbox_data(self, json_path, img):
        ...
        for obj in labels:
            ...
            if labels[obj]['class_id'] == 'human':
                ...
                instances.append(obj)  # Stores the instance id
                bboxes.append([x1, y1, x2, y2])  # Stores the bbox coordinates
        ...
        for box_pair in list(itertools.combinations(range(len(bboxes)), 2)):
            ...
            if overlap1 >= 0.6 or overlap2 >= 0.6:
                # Determines which of the two bboxes has a higher distance
                if labels[instances[box_pair[0]]]['distance'] > labels[instances[box_pair[1]]]['distance']:
                    ignore[box_pair[0]] = 1
                else:
                    ignore[box_pair[1]] = 1

        return instances, bboxes, class_ids, bbox_areas, ignore
