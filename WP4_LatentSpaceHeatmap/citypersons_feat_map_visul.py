...
def visualize_new(folder_name, img_path, activations, img, output, target=None):
    # input images
    img_255_tensor = img * 255
    img_255_tensor = img_255_tensor.to(torch.device("cpu"), torch.uint8)
    img_255 = img_255_tensor.numpy().transpose(1, 2, 0)
...
def inference(model, device, folder_name, feature_maps, img_list):
    ...
    for idx, img in tqdm.tqdm(enumerate(img_list)):
        ...
        visualize_new(folder_name, img, activations, image, results[0])
...
def main():
    ...
    cfg_path = '/path/to/eval_config.yaml'
    ...
    root_path = '/path/to/CityPersons/demoVideo/stuttgart_00'
    img_path_list = path_list(root_path)

    inference(model, device, folder_name, feature_maps, img_list=img_path_list)
