...
from fcos_torchvision.contrastive_module import ContrastiveLoss
...
class FCOSHead(nn.Module):
    ...
    def compute_loss(
        ...
        # compute foregroud over the whole batch size
        ...
        )  # not on the whole image but only the foreground
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        # add the contrastive loss calculation module
    ...
    def forward(
        ...
        features = self.backbone(images.tensors)  # TODO: add contrastive loss
        ...
        if self.training:
            ...
            contrastive_loss = self.contrastive_loss(features, images.image_sizes, targets)
        else:
            contrastive_loss = {}
        ##################################################################
        ...
        if self.training:
            ...
            ##################################################################
            losses.update(contrastive_loss)  # add the contrastive loss
            ##################################################################
        else:
