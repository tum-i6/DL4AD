...
import utils
import pandas as pd
from evaluation_monitoring import evaluate_model_by_heatmap_vs_bbox_main
...
def load_dataloaders(cfg):
    ...
    if cfg['dataset'] == 'KIA':
        ...
        train_loader, val_loader, test_loader, cfg = load_citypersons_dataloaders(cfg)
...
def load_citypersons_dataloaders(cfg):
    ...
    train_indices = []
    test_indices = []
    train_df=pd.DataFrame(columns=['sample_idx', 'img_sample_path'])
    ...
    for sample_idx, img_sample_path in enumerate(train_dataset.imgs):
        ...
        if split == 'train':
            ...
            train_df.loc[sample_idx]={
                'sample_idx': sample_idx,
                'img_sample_path': img_sample_path
            }
        elif split == 'val':
            test_indices.append(sample_idx)
    # Samples randomly 300 train samples to be used for the validation set
    cfg['train_length'] = len(train_indices)
    if cfg['round']==1:
        train_df = train_df.sample(frac=1).reset_index(drop=True)
        val_df = train_df.iloc[:301].reset_index(drop=True)
        train_df = train_df.iloc[301:].reset_index(drop=True)
        selection_subset = int(cfg['train_split_start'] * cfg['train_length'])
        train_subset1_df=train_df.iloc[:selection_subset].reset_index(drop=True)
        train_subset2_df=train_df.iloc[selection_subset:].reset_index(drop=True)
        train_indices=list(train_subset1_df['sample_idx'].values)
        val_indices=list(val_df['sample_idx'].values)
    else:
        val_df=pd.read_csv(cfg['val_inds'], delimiter=',')
        train_subset1_df = pd.read_csv(cfg['round1_sub1_inds'], delimiter=',')
        train_subset2_df = pd.read_csv(cfg['round1_sub2_inds'], delimiter=',')
        selection_subset=int(cfg['train_added_percentage']*cfg['train_length'])
        if cfg['train_added_method']=='random':
            train_subset2_df=train_subset2_df.sample(frac=1).reset_index(drop=True)

        elif cfg['train_added_method']=='heatmap':
            train_subset2_df=evaluate_model_by_heatmap_vs_bbox_main(cfg, train_subset2_df)
            train_subset2_df=train_subset2_df.sort_values('iou')
            cfg['pretrained_weights_path']=False
        elif cfg['train_added_method']=='confidence':
            train_subset2_df= evaluate_model_by_heatmap_vs_bbox_main(cfg, train_subset2_df)
            train_subset2_df=train_subset2_df.sort_values('score')
            cfg['pretrained_weights_path']=False

        train_subset2_df_picked = train_subset2_df.iloc[:selection_subset].reset_index(drop=True)
        train_subset2_df = train_subset2_df.iloc[selection_subset:].reset_index(drop=True)

        train_subset1_df=train_subset1_df.append(train_subset2_df_picked, ignore_index=True)
        train_subset1_df = train_subset1_df.sample(frac=1).reset_index(drop=True)
        train_indices = list(train_subset1_df['sample_idx'].values)
        val_indices = list(val_df['sample_idx'].values)
    train_subset1_df.to_csv(f"{cfg['session_path']}/train_sub1_indices.csv")
    train_subset2_df.to_csv(f"{cfg['session_path']}/train_sub2_indices.csv")
    val_df.to_csv(f"{cfg['session_path']}/val_indices.csv")
