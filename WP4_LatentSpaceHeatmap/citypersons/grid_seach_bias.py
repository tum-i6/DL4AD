...
def boost_scores_offline(prob_map, output, device, alpha, beta, bias=0, conf_threshold=0.05):
    copied_output = output.copy()
    # convert boxes to integer type
    ...
    for origi_box, box, score in zip(copied_output['boxes'], output['boxes'], output['scores']):
        ...
        if area == 0:
            ...
            averaged_score = (score.item()+2*bias) / (1 + bias + np.exp(-(np.mean(prob_map[y1:y2, x1:x2]) - beta) * alpha))  # (5
            averaged_score = max(0, min(averaged_score, 1))
...
def evaluate_model_grid_search_offline(model, data_loader, device, alpha, beta, bias=0, conf_threshold_post_pros=None, feat_maps=None, dataframe=None):
    ...
    for index, row  in tqdm(dataframe.iterrows(), total=len(dataframe)):
        ...
        outputs = [boost_scores_offline(p_map, output, device, alpha, beta, bias=bias, conf_threshold=conf_threshold_post_pros)]
...
def main():
    ...
    if model_name == 'FasterRCNN' and dataset_name == "CityPersons":
        ...
        beta_tuning = np.arange(0, 0.3, .05)  # 21

    alpha_tuning = np.arange(1, 10.5, 0.5)  # 19
    alpha_tuning = np.arange(13, 20, 1)  # 19
    bias_tuning=np.arange(0,0.3,0.02)
    #bias_tuning=np.arange(0.1,0.,0.1)
