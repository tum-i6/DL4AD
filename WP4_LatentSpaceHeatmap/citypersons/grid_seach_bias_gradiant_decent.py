...
class GridSearchTorch(nn.Module):
    """
    ...
    def __init__(self):
        super().__init__()
        self.alpha = nn.Parameter(torch.tensor(torch.rand(1), dtype=torch.float, requires_grad=True))
        self.beta = nn.Parameter(torch.tensor(torch.rand(1), dtype=torch.float, requires_grad=True))
        self.bias = nn.Parameter(torch.tensor(torch.rand(1), dtype=torch.float, requires_grad=True))
        #self.nn_layers = nn.ModuleList()
    def boost(self,output, prob_map,conf_threshold=0.5):
        ...
        for box, score in zip(output['boxes'], output['scores']):
            ...
            if area == 0:
                ...
                p_mean=torch.mean(prob_map[y1:y2, x1:x2])
                averaged_score = (score + self.bias) / (
                        1 + self.bias + torch.exp(-(p_mean - self.beta) * self.alpha))  # (5
                torch.max(torch.tensor(0), torch.min(averaged_score, torch.tensor(1.)))
                # averaged_score = (score+bias) / (1 + bias + torch.exp(-(torch.mean(prob_map[y1:y2, x1:x2]) - beta) * alpha))  # (5
                # averaged_score = torch.max(0, torch.min(averaged_score, 1)).item()

            adjusted_scores.append(averaged_score)

        output['scores'] = torch.tensor(adjusted_scores, dtype=output['scores'].dtype)

        copied_output = {k: v[output['scores'] > conf_threshold] for k, v in output.items()}
        return copied_output
    def calculate_iou(self, bbox1, bbox2):
        x1_overlap = torch.max(bbox1[0], bbox2[0])
        y1_overlap = torch.max(bbox1[1], bbox2[1])
        x2_overlap = torch.min(bbox1[2], bbox2[2])
        y2_overlap = torch.min(bbox1[3], bbox2[3])

        intersection_area = torch.clamp(x2_overlap - x1_overlap + 1, min=0) * torch.clamp(y2_overlap - y1_overlap + 1, min=0)
        ...
        return iou
    def forward(self, model_outputs, target_bboxes, prob_map):


        model_outputs=self.boost(model_outputs,prob_map)

        true_positives = 0
        false_positives = 0
        for output_bbox in model_outputs['boxes']:
            for target_bbox in target_bboxes:
                if self.calculate_iou(output_bbox, target_bbox) >= 0.5:
                    true_positives += 1
                    break  # Break once a true positive is found

        false_positives = len(model_outputs) - true_positives

        precision = true_positives / (true_positives + false_positives) if true_positives + false_positives > 0 else 0
        return model_outputs, torch.tensor(1-precision)
class PrecisionLoss(torch.nn.Module):
    def __init__(self, threshold=0.5):
        super(PrecisionLoss, self).__init__()
        self.threshold = threshold

    def forward(self, model_outputs, target_bboxes):
        true_positives = 0
        false_positives = 0

        for output_bbox in model_outputs:
            for target_bbox in target_bboxes:
                if self.calculate_iou(output_bbox, target_bbox) >= self.threshold:
                    true_positives += 1
                    break  # Break once a true positive is found
        ...
        # Return negative precision to minimize it (assuming this is a loss term)
        return torch.tensor(-precision, dtype=torch.float32, requires_grad=True)
    ...
    def calculate_iou(self, bbox1, bbox2):
        ...
        return iou
def load_optimizer(model,optimizer= 'SGD'):
    """
...
def boost_scores_offline(prob_map, output, device, alpha, beta,grid_search_model, bias=0,  conf_threshold=0.05):
    copied_output = output.copy()
    # convert boxes to integer type
    ...
    for origi_box, box, score in zip(copied_output['boxes'], output['boxes'], output['scores']):
        ...
        if area == 0:
            ...
            averaged_score=grid_search_model(score,torch.mean(prob_map[y1:y2, x1:x2]))
            #averaged_score = (score+bias) / (1 + bias + torch.exp(-(torch.mean(prob_map[y1:y2, x1:x2]) - beta) * alpha))  # (5
            #averaged_score = torch.max(0, torch.min(averaged_score, 1)).item()
...
def evaluate_model_grid_search_offline(model, data_loader, device, alpha, beta, bias=0, conf_threshold_post_pros=None, feat_maps=None, dataframe=None):
    """
    Iterates over all sample images from the data_loader and infers them into the model to evaluate the model's
    prediction performance. Logs the model's inference time and the final evaluation results to the console.
    ...
    grid_search_model=GridSearchTorch()

    # Define the optimizer (stochastic gradient descent)
    optimizer = optim.SGD(grid_search_model.parameters(), lr=0.01)
    # Define the loss function (mean squared error)
    criterion = nn.MSELoss()
    precision_loss = PrecisionLoss()
    grid_search_model.train()
    ...
    with profiler.profile(record_shapes=True, use_cuda=False) as prof:
        ...
        for index, row  in tqdm(dataframe.iterrows(), total=len(dataframe)):
            ...
            # probability/ likelihood map with value 0-1 by sigmoid func
            p_map = torch.tensor(row['heatmap'])
            ...
            #outputs = [boost_scores_offline(p_map, output, device, alpha, beta,grid_search_model=grid_search_model, bias=bias, conf_threshold=conf_threshold_post_pros)]
            outputs, loss=grid_search_model(output, target['boxes'], p_map)
            ...
            try:
                precision=eval_results["0.5"]["0.5"]['TPs']/(eval_results["0.5"]["0.5"]['TPs']+eval_results["0.5"]["0.5"]['FPs'])
            except:
                precision=0
            # Compute the loss
            #loss = criterion(torch.tensor(precision), torch.tensor(1.0))

            loss = precision_loss(output['boxes'], target['boxes'])
...
def main():
    ...
    saving_path = f'/path/to/grid_search_score_boosting/{tag}'
    ...
    if model_name == 'FasterRCNN' and dataset_name == "CityPersons":
        beta_tuning = np.arange(0, 0.425, 0.025)
    else:
        beta_tuning = np.arange(0, 1.05, .05)  # 21
        beta_tuning = np.arange(0, 1.1, .2)  # 21

    alpha_tuning = np.arange(1, 10.5, 0.5)  # 19
    alpha_tuning = np.arange(1, 15, 1)  # 19
    bias_tuning=np.arange(0,1,0.01)
    bias_tuning=np.arange(0.1,1,0.1)
    coco_ap_matrix = np.zeros((len(beta_tuning), len(alpha_tuning)))
    pascal_ap_matrix = np.zeros((len(beta_tuning), len(alpha_tuning)))
    lamr_matrix = np.zeros((len(beta_tuning), len(alpha_tuning)))

    precision_matrix = np.zeros((len(beta_tuning), len(alpha_tuning)))
    recall_matrix = np.zeros((len(beta_tuning), len(alpha_tuning)))
    ...
    if int(contra_weighting_factor) != 0:
        ...
        print('test-set')
        for idx_bias, bias in enumerate(bias_tuning):
            for idx_alpha, alpha in enumerate(alpha_tuning):
                for idx_beta, beta in enumerate(beta_tuning):
                    print(f'bias {bias}, alpha {alpha}, beta {beta}')
                    ...
                    if not os.path.exists(f'{saving_path}/train_dataframe.p'):
                        ...
                        results_score_boost, df = evaluate_model_grid_search(model_zero_conf, train_loader, device, b=alpha, c=beta,
                        ...
                        results_score_boost= evaluate_model_grid_search_offline(model_zero_conf, train_loader, device, alpha=alpha,
                                                                                beta=beta,
                                                                                bias=bias,
                                                                                conf_threshold_post_pros=float(
                                                                                 standard_conf_thres[cfg['model_name']]),
                    ...
                    coco_ap_matrix[idx_beta, idx_alpha] = coco_ap
                    pascal_ap_matrix[idx_beta, idx_alpha] = pascal_ap
                    lamr_matrix[idx_beta, idx_alpha] = lamr
    
                    precision_matrix[idx_beta, idx_alpha] = round(precision, 3)
                    recall_matrix[idx_beta, idx_alpha] = round(recall, 3)
                    print(f'Coco AP: {coco_ap}, Pascal AP: {pascal_ap}, LAMR:{lamr}')
                # print(coco_ap_matrix)
                # print(recall_matrix)
    
            plt.subplots(figsize=(20, 15))
            ax0 = sns.heatmap(coco_ap_matrix, xticklabels=alpha_tuning, yticklabels=beta_tuning, cmap="YlGnBu", annot=True, fmt='.3g')
            ax0.set_title(f'coco_ap_matrix\n{tag}')
            plt.xlabel('b')
            plt.ylabel('c')
            plt.savefig(f'{saving_path}/{tag}_coco_ap_bias={bias}.png')
            plt.close()
        
            #np.save(f'{saving_path}/{tag}_coco_ap_array.npy', coco_ap_matrix)
        
            plt.subplots(figsize=(20, 15))
            ax1 = sns.heatmap(pascal_ap_matrix, xticklabels=alpha_tuning, yticklabels=beta_tuning, cmap="YlGnBu", annot=True, fmt='.3g')
            ax1.set_title(f'pascal_ap_matrix\n{tag}')
            plt.xlabel('b')
            plt.ylabel('c')
            plt.savefig(f'{saving_path}/{tag}_pascal_ap_bias={bias}.png')
            plt.close()
        
            #np.save(f'{saving_path}/{tag}_pascal_ap_array.npy', pascal_ap_matrix)
        
            plt.subplots(figsize=(20, 15))
            ax2 = sns.heatmap(lamr_matrix, xticklabels=alpha_tuning, yticklabels=beta_tuning, cmap="YlGnBu", annot=True, fmt='.3g')
            ax2.set_title(f'lamr_matrix\n{tag}')
            plt.xlabel('b')
            plt.ylabel('c')
            plt.savefig(f'{saving_path}/{tag}_lamr_bias={bias}.png')
            plt.close()
        
            #np.save(f'{saving_path}/{tag}_lamr_array.npy', lamr_matrix)
        
            plt.subplots(figsize=(20, 15))
            ax3 = sns.heatmap(precision_matrix, xticklabels=alpha_tuning, yticklabels=beta_tuning, cmap="YlGnBu", annot=True, fmt='.3g')
            ax3.set_title(f'precision_matrix\n{tag}')
            plt.xlabel('b')
            plt.ylabel('c')
            plt.savefig(f'{saving_path}/{tag}_precision_bias={bias}.png')
            plt.close()
        
            #np.save(f'{saving_path}/{tag}_precision_array.npy', precision_matrix)
        
            plt.subplots(figsize=(20, 15))
            ax4 = sns.heatmap(recall_matrix, xticklabels=alpha_tuning, yticklabels=beta_tuning, cmap="YlGnBu", annot=True, fmt='.3g')
            ax4.set_title(f'recall_matrix\n{tag}')
            plt.xlabel('b')
            plt.ylabel('c')
            plt.savefig(f'{saving_path}/{tag}_recall_bias={bias}.png')
            plt.close()
