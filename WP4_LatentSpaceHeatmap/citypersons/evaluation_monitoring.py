...
def boost_scores_method4(prob_map, output, b, c, conf_threshold=0.05):
...
def evaluate_model(model, data_loader, device, conf_threshold_post_pros=None, feat_maps=None, cfg=None):
    ...
    for batch_idx, (images, targets) in enumerate(tqdm(data_loader, desc="Evaluating")):
        ...
        if feat_maps is not None:
            ...
            if len(images) != 1:
                ...
                outputs = [boost_scores_method4(p_map, output,cfg['b'],cfg['c'], conf_threshold=conf_threshold_post_pros)]
    ...
    print(f'Mean Recall                 (R)     @[ IoU=0.25           &  C=0.0:0.05:1.0 ] = {round(mean_recall, 3)}')
    print(
        f'Mean Precision              (P)     @[ IoU=0.25           &  C=0.0:0.05:1.0 ] = {round(mean_precision, 3)}')
    print(
...
@torch.inference_mode()
def evaluate_model_by_heatmap_vs_bbox_main(cfg, data_df):
    """
    Iterates over all sample images from the data_loader and infers them into the model to evaluate the model's
    prediction performance. Returns the results including the IoU between the predicted bboxes and the heatmaps
    ...
    feature_maps = []
    handle = model.backbone.register_forward_hook(lambda module, input,
                                                                   output: feature_maps.append(output))
    # Iterates over all batches from the data_loader
    data_df['iou'] = 0
    data_df['score'] = 0
    print(cfg['pretrained_weights_path'])
    with torch.no_grad():
        #for batch_idx, (images, targets) in enumerate(metric_logger.log_every(data_loader, 100, "Test:")):
        for row_idx, row in enumerate(tqdm(data_df.iterrows(), desc="Evaluating Unseen Train Samples", total=len(data_df))):

            # Moves the image samples to the GPU
            images = Image.open(f"{cfg['data_path']}/{row[1]['img_sample_path']}").convert("RGB")
            ...
            if feature_maps is not None:
                ...
                if cfg['model_name'] != 'SSD300-ResNet50-OD':
                    ...
                    heatmap = torch.mean(activations, 1)#.squeeze(0).numpy()
                #resized_heatmap = cv2.resize(heatmap, (i_w, i_h))
                resized_heatmap = torchvision.transforms.Resize(size=(i_h, i_w), antialias=True)(heatmap)[0]

                # probability/ likelihood map with value 0-1 by sigmoid func
                #p_map = 1 / (1 + np.exp(-resized_heatmap * 2))
                #p_seg = np.where(p_map > 0.5, 1, 0)
                p_map = torch.nn.Sigmoid()(resized_heatmap) > 0.5
                p_seg = p_map.long()  # (H, W)
                boxes=output['boxes'].cpu().numpy()
                scores=output['scores'].cpu().numpy()
                output_mask=np.zeros((i_w, i_h))
                for i,  box in enumerate(boxes):
                    if scores[i] >= cfg['conf_threshold']:
                        # label smoothing for pos. pixels: 0.9 instead of 1
                        output_mask[int(box[0]): int(box[2]), int(box[1]): int(box[3])] = 1
                output_mask=torch.from_numpy(output_mask).long().T
                pos_intersection = (p_seg & output_mask).float().sum()  # TP
                pos_union = (p_seg | output_mask).float().sum()  # TP+FP+FN
                pos_iou=pos_intersection/pos_union

                neg_intersection = ((1 - p_seg) & (1 - output_mask)).float().sum()  # TF
                neg_union = ((1 - p_seg) | (1 - output_mask)).float().sum()  # TF+FP+FN
                neg_iou=neg_intersection/neg_union

                avg_iou=(pos_iou+neg_iou)/2


                data_df.loc[row_idx, 'iou'] = avg_iou.item()
                data_df.loc[row_idx, 'score'] = np.mean(scores)

    #del model
    #del feature_maps, boxes, scores, p_seg, output_mask, pos_iou, pos_union, neg_intersection, pos_intersection
    #del output, outputs, resized_heatmap, avg_iou
...
def enable_dropout(model):
    ...
    for m in model.modules():
        if m.__class__.__name__.startswith('Dropout'):
            m.train()
@torch.inference_mode()
def evaluate_model_heatmap_vs_score_main(cfg, data_loader):
    """
    Iterates over all sample images from the data_loader and infers them into the model to evaluate the model's
    prediction performance. Returns the results including the IoU between the predicted bboxes and the heatmaps
    ...
    with torch.no_grad():
        #for batch_idx, (images, targets) in enumerate(metric_logger.log_every(data_loader, 100, "Test:")):
        #for batch_idx, (images, targets)in enumerate(tqdm(data_loader, desc="Evaluating Unseen Train Samples", total=len(data_loader))):
        for batch_idx, (images, targets)in enumerate(tqdm(data_loader, desc="Evaluating Unseen Train Samples", total=min(len(data_loader), 3000))):
            if batch_idx>3000:
                break
            feature_maps = []
            #handle = model.backbone.register_forward_hook(lambda module, input,
            ...
            for t in targets:
                target={}
                for k, v in t.items():
                    if k=='boxes' or k=='labels':
                        target[k]=v.to(device)
                targets_gpu.append(target)
            ...
            # Iterates over all evaluation thresholds to compute the respective precision, recall, F1-score, MR and FPPI
            for conf_threshold in batch_eval_results:
                for iou_threshold in batch_eval_results[conf_threshold]:
                    tps = batch_eval_results[conf_threshold][iou_threshold]['TPs']
                    srtps = batch_eval_results[conf_threshold][iou_threshold]['SRTPs']
                    fps = batch_eval_results[conf_threshold][iou_threshold]['FPs']
                    fns = batch_eval_results[conf_threshold][iou_threshold]['FNs']

                    try:
                        precision = tps / (tps + fps)
                    except ZeroDivisionError:
                        precision = 0.0  # This should only occur when conf_threshold = 1.0 (no predictions)
                    try:
                        recall = srtps / (srtps + fns)
                        recall = tps / (tps + fns)
                        miss_rate = fns / (srtps + fns)
                        miss_rate = fns / (tps + fns)
                    except ZeroDivisionError:
                        recall=0.0
                        miss_rate=0.0
                    fppi = fps / len(data_loader.dataset)
                    ...
                    # Stores the computed precision, recall, miss rate, F1-score and fppi values
                    batch_eval_results[conf_threshold][iou_threshold]['precision'] = precision
                    batch_eval_results[conf_threshold][iou_threshold]['recall'] = recall
                    batch_eval_results[conf_threshold][iou_threshold]['F1'] = f1
                    batch_eval_results[conf_threshold][iou_threshold]['MR'] = miss_rate
                    batch_eval_results[conf_threshold][iou_threshold]['FPPI'] = fppi
            # Computes the average precision values for multiple IoU thresholds (25 to 95 with a step-size of 5)
            aps = utils.get_average_precisions(batch_eval_results)
            coco_ap = round(sum([aps[iou_thr] for iou_thr in aps if float(iou_thr) >= 0.5]) / 10,
                            3)  # COCO AP metric
            pascal_voc_ap = round(aps['0.5'], 3)  # PASCAL VOC AP metric
            lamr = round(utils.get_log_avg_miss_rate(batch_eval_results, '0.5'), 3)  # LAMR metric
            ...
            with torch.no_grad():
                for i in range(20):
                    ...
                    torch.cuda.empty_cache()
                    scores_dropout.append(outputs[0]['scores'].mean().cpu().numpy())

            scores_dropout = np.stack(scores_dropout)
            results_df.loc[batch_idx]={
                'sample_idx': batch_idx,
                'iou': pos_iou.item(),
                'score': np.mean(scores),
                'score_mc': np.mean(scores_dropout),
                'AP_coco': coco_ap,
                'AP_pascal': pascal_voc_ap,
                'LAMR': lamr
            }
            #print(results_df)
            model.eval()
...
def main():
    ...
    parser.add_argument('--mode', default='normal', help='boosted, normal')
    ...
    if not args.no_eval:
        ...
        if args.mode=='normal':
            ...
            results = evaluate_model_by_heatmap_vs_bbox(model, dataset, device)
            ...
            if int(contra_weighting_factor) != 0:
                ...
                print(f"Params: b: {cfg['b']}, c: {cfg['c']}")
                ...
                handle = model_zero_conf.backbone.register_forward_hook(lambda module, input,
                                                                               output: feature_maps.append(output))
                # Evaluates the model weights from the checkpoint file on the test set
                results_score_boost, scores = evaluate_model(model_zero_conf, dataset, device,
                                                     conf_threshold_post_pros=float(standard_conf_thres[cfg['model_name']]),
...
def main2():
    ...
    if args.subset == 'train':
        ...
        print(f'Please make sure the subset argument can only take "train", "val" or "test" as input.')
        exit()
    results_df= evaluate_model_heatmap_vs_score_main(cfg, test_loader)
    results_df.to_csv(cfg['pretrained_weights_path'].replace(cfg['pretrained_weights_path'].split('/')[-1], 'correlation_results.csv'))

if __name__ == '__main__':
    from plf_analysis_plot import plot_average_precisions, plot_log_avg_miss_rate
