...
import torch
import yaml
from data_monitoring import load_dataloaders
import utils
from evaluation_monitoring import evaluate_model
...
def train_model(cfg, model, contra_weight, device, optimizer, lr_scheduler, train_loader, val_loader, session_path, starting_epoch, writer):
    ...
    for epoch in range(starting_epoch, cfg['num_epochs']):
        ...
        train_metrics = train_one_epoch(model, contra_weight, optimizer, train_loader, device, epoch, cfg, print_freq=10)
        lr_scheduler.step()

        # Evaluates the current model weights on the validation dataset
        eval_results, _= evaluate_model(model, val_loader, device)
...
def train_one_epoch(model, contra_weight, optimizer, data_loader, device, epoch, cfg, print_freq):
    """
    ...
    with torch.autograd.set_detect_anomaly(True):
        #for images, targets in tqdm(metric_logger.log_every(data_loader, print_freq, header=f"Epoch: [{epoch}]")):
        for images, targets in tqdm(data_loader, total=len(data_loader),
                                    desc=f"Round={cfg['round']}, Epoch {epoch}, Picked Data={(cfg['train_split_start']+cfg['train_added_percentage']*(cfg['round']-1))*100}%"):
...
def evaluate_train_session(cfg, model, device, test_loader, session_path):
    ...
    eval_results,_ = evaluate_model(model, test_loader, device)
    ...
    # Evaluates the model weights from the checkpoint file on the test set
    eval_results, scores = evaluate_model(model, test_loader, device)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='../config/train_config_monitoring.yaml',
    ...
    if args.resume:
        ...
        session_name = f"{str(datetime.now())[:-7].replace(' ', '_').replace(':', '-')}_{cfg['dataset']}_{cfg['model_name']}"
        session_name = f"{cfg['dataset']}_{cfg['model_name']}_selection_{cfg['train_added_method']}_round_{cfg['round']}"
    ...
    train_loader, val_loader, test_loader, cfg= load_dataloaders(cfg)
    ...
    # Here starts the active learning stuff!
    num_rounds= int((1-cfg['train_split_start'])/cfg['train_added_percentage'])+1
    train_df = pd.DataFrame(columns=['round', 'train_coverage', 'AP_COCO', 'AP_Pascal', 'LAMR'])
    for ind, round in enumerate(range(1,num_rounds+1)):
        ...
        if round==1:
            ...
            cfg['round']=round
            cfg['round1_sub1_inds']=f"{cfg['session_path']}/train_sub1_indices.csv"
            cfg['round1_sub2_inds']=f"{cfg['session_path']}/train_sub2_indices.csv"
            cfg['val_inds']=f"{cfg['session_path']}/val_indices.csv"
            cfg['pretrained_weights_path']=f"{cfg['session_path']}/final_model_coco.pth"
            session_name = f"{cfg['dataset']}_{cfg['model_name']}_selection_{cfg['train_added_method']}_round_{cfg['round']}"
            session_path = f"{cfg['output_path']}/{session_name}_weight_{int(contra_weight.item() * 100)}"
            cfg['session_path'] = session_path
            if not os.path.exists(session_path):
        ...
        eval_results=evaluate_train_session(cfg, model, device, test_loader, session_path)
        train_df.loc[ind] = {
            'round': round,
            'train_coverage': (cfg['train_split_start']+cfg['train_added_percentage']*(round-1))*100,
            'AP_COCO': eval_results['coco_ap'],
            'AP_Pascal': eval_results['pascal_voc_ap'],
            'LAMR': eval_results['lamr']

        }
        train_df.to_csv(f"{cfg['output_path']}/{cfg['dataset']}_{cfg['model_name']}_selection_{cfg['train_added_method']}_test_results_seed_{cfg['seed']}.csv")
