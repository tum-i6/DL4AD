...
def evaluate_detection(results_path, det_path, gt_path, det_method_name, eval_type='pedestrian'):
    ...
    print(dict(zip(['reasonable', 'small', 'occluded', 'all'], results)))


def eval(root, time='day', mode='val', eval_type='pedestrian', det_path=None):
    assert time in ['day', 'night']
    assert mode in ['val', 'test']

    gt_path = '{}/ECP/{}/labels/{}'.format(root, time, mode)
    if det_path is None:
    ...
    results_path = '/'.join(det_path.split('/')[:-3])
